...
### 混沌工程与自动化灾备演练 (Automated Game Days)
为了防止灾备方案在关键时刻失效（如配置漂移或云厂商临时封锁），我们引入了自动化混沌工程演练。
- **周末消防演习**: 通过 GitHub Actions 流水线 (`dr_game_day.yml`)，系统在每周日凌晨 3 点自动通过 SSH 登录我们的悉尼备用灾备机房，一键拉起全新的 Docker 容器集群，并运行全套的 Ragas 质量门禁端到端测试。
- **API 封锁熔断预警**: 如果 DeepSeek 或 Gemini 突然封锁了悉尼区域的 IP，周日早晨的演练会自动阻断并发出警报，让团队在真实灾难发生前提前感知并切换备用模型通道。测试通过后，流水线会自动销毁拉起的容器组，以最大限度节省云端算力成本。
