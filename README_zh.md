...
一个基于微服务架构的 SEC 财报分析工具。它通过 FastAPI、Celery 和 Milvus 实现全异步的文档检索，并通过自动化 Ragas 审计确保提取出来的每一个财务指标都能在财报原文中找到出处。

⚡ **核心设计与防幻觉逻辑：**
- **用 Milvus 锁定真实数据**：我们不让大模型凭空联想数字。系统解析财报后，将文本切片存入 **Milvus 向量数据库**。每次提问时，系统只把最相关的财报原句喂给大模型，强制其仅基于原文进行逻辑推理。
- **报告出厂前的 Ragas 自动审计**：生成的报告必须通过 Ragas 指标（真实度、相关度、召回率）的自动化跑分测试，确保大模型没有偷换财报数据。
- **全异步任务队列**：采用 **FastAPI、Celery、RabbitMQ 和 Redis** 搭建任务分发管线，将耗时的财报解析和向量化放到后台执行，防止前端页面卡死。
...
| ID | 需求描述 | 类型 | SLA / 指标 | 状态 |
| :--- | :--- | :--- | :--- | :--- |
| FR-01 | 解析 SEC 10-K / 10-Q 财报 PDF，提取表格与文本 | 功能需求 (P0) | — | ✅ |
| FR-02 | 自然语言问答，用户可直接检索财报数据 | 功能需求 (P0) | — | ✅ |
| FR-03 | 每个回答提供可追溯的引用来源 | 功能需求 (P0) | — | ✅ |
| FR-04 | 异步文档处理，上传后无需等待 | 功能需求 (P1) | — | ✅ |
| FR-05 | 提供 RAGAS 审计报告 | 功能需求 (P1) | — | ✅ |
| NFR-01 | 性能 | 非功能需求 | P99 < 200ms (缓存); 首 Token < 1.5s | ✅ Redis 语义缓存 |
| NFR-02 | 可用性 | 非功能需求 | SLA >= 99.9%; RTO < 10min; RPO < 15min | ✅ LLM Cascade + Pilot Light |
| NFR-03 | 准确性 | 非功能需求 | Faithfulness >= 0.85; Precision >= 0.75 | ✅ CI/CD 质量门禁 |
| NFR-04 | 安全 | 非功能需求 | 零信任架构; 密钥永不硬编码 | ✅ Tailscale + KMS |
| NFR-05 | 可扩展性 | 非功能需求 | 水平扩展 (Milvus, Celery 独立扩容) | ✅ 微服务 + Docker |

### 质量保障策略 (QA Strategy)
- **单元测试** — `pytest` 覆盖核心解析逻辑与 API 路由
- **集成测试** — Docker Compose 全环境端到端验证
- **性能压测** — Locust 并发模拟; 验证缓存命中率与 P99 延迟
- **混沌工程** — 每周自动化 DR 演练 (`dr_game_day.yml`); 验证 RTO/RPO
- **生产 AIOps** — Prometheus + Grafana 实时大盘; LLM Cascade 异常自动切换
...
基于腾讯云部署场景，在不降低性能的前提下，从以下维度控制云资源成本：

### 10.1 计算层：Serverless 化与弹性伸缩
- **推荐方案**：对于 AI 推理这类负载波动明显的业务，优先使用 TDSQL-C Serverless 模式与函数计算 (SCF)。
- **预计收益**：相比长期持有固定规格的 CVM，可将计算闲置成本压缩 **60%-80%**。

### 10.2 存储层：冷热数据分层
- **推荐方案**：历史财报解析结果（如超过 6 个月未查询的文档）自动沉降到 COS 归档存储。
- **预计收益**：相比全量使用 SSD 云硬盘，归档存储成本降低约 **85%**。

### 10.3 数据库层：TDSQL-C vs 自建 PostgreSQL

| 场景 | 描述 |
| :--- | :--- |
| **CVM 自建 PG** | 需预付 CVM 费用 + 自配主从 + 自调优 + 自备灾 |
| **托管 TDSQL-C** | 按实际计算 (CCU) 和存储 (GB) 使用量付费，自带高可用与自动备份 |

**结论**：TDSQL-C 通过"计算存储分离"，仅需为实际消耗的资源买单，综合 TCO 预估可降低 **30%-40%**。

### 10.4 购买策略
- **开发/测试环境**：按量计费，随建随删。
- **生产环境**：包年包月 + 资源包预购，相比按量计费可节省约 **36%-50%** 的年度支出。

---

## 🔹 11. 出海竞争力：腾讯云差异化优势

在海外业务拓展中，腾讯云的差异化价值体现在「场景化方案」而非单纯的算力价格比拼：

| 场景 | 腾讯云优势 | 典型行业 |
| :--- | :--- | :--- |
| 实时音视频 (TRTC) | 源于 QQ/微信的技术积累，超低延时，抗弱网能力强 | 直播电商、在线教育、社交娱乐 |
| 游戏出海 | 一站式解决方案（加速、安全、语音），深度理解游戏生命周期痛点 | 游戏发行、电竞平台 |
| 数据库 (TDSQL) | 金融级强一致性，且在国产化替代浪潮中拥有大量行业案例 | 泛金融、SaaS 服务商 |

---

## 🔹 12. 托管数据库 vs Docker 自建 PostgreSQL（架构决策）

| 维度 | Docker 自建 PG（当前模式） | 腾讯云托管数据库（如 TDSQL-C） |
| :--- | :--- | :--- |
| **高可用** | 需自行搭建主从及故障转移 | 自带高可用架构，跨可用区部署 |
| **备份恢复** | 自行编写脚本与管理备份文件 | 自动备份，一键回档至任意时间点 |
| **性能调优** | 需自行分析慢查询与参数调优 | 控制台提供性能诊断与优化建议 |
| **弹性扩容** | 需停机或自行设计在线方案 | 控制台操作，在线秒级平滑扩容 |
| **成本结构** | CVM 租赁费 + DBA 人力运维成本 | 按规格付费（或 Serverless 按量付费），无需专职 DBA |

> **SA 核心观点**：托管服务的"省钱"不仅体现在资源费用上，更重要的是**解放了客户的运维人力**，让团队可以把精力集中在业务创新上。
